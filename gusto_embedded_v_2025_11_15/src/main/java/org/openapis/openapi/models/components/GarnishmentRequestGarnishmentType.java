/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package org.openapis.openapi.models.components;

import com.fasterxml.jackson.annotation.JsonValue;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;

/**
 * GarnishmentRequestGarnishmentType
 * 
 * <p>The specific type of garnishment for court ordered garnishments.
 */
public enum GarnishmentRequestGarnishmentType {
    CHILD_SUPPORT("child_support"),
    FEDERAL_TAX_LIEN("federal_tax_lien"),
    STATE_TAX_LIEN("state_tax_lien"),
    STUDENT_LOAN("student_loan"),
    CREDITOR_GARNISHMENT("creditor_garnishment"),
    FEDERAL_LOAN("federal_loan"),
    OTHER_GARNISHMENT("other_garnishment");

    @JsonValue
    private final String value;

    GarnishmentRequestGarnishmentType(String value) {
        this.value = value;
    }
    
    public String value() {
        return value;
    }
    
    public static Optional<GarnishmentRequestGarnishmentType> fromValue(String value) {
        for (GarnishmentRequestGarnishmentType o: GarnishmentRequestGarnishmentType.values()) {
            if (Objects.deepEquals(o.value, value)) {
                return Optional.of(o);
            }
        }
        return Optional.empty();
    }
}

